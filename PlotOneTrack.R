...
#without tracks
a<-mxy %>% filter(Animal==131127)

d<-SpatialPointsDataFrame(cbind(a$x,a$y),data=a,proj4string=CRS("+proj=longlat +datum=WGS84"))
ggplot(a,aes(x=x,y=y)) + borders(fill="grey") + coord_cartesian(xlim=as.numeric(bbox(d)[1,]),ylim=as.numeric(bbox(d)[2,])) + theme_bw()  + geom_path(size=1)
