...
ggplot(timef,aes(x=Month,y=Days,fill=phistate)) + geom_boxplot() + theme_bw() + facet_wrap(~phistate,scales="free")
ggsave("Figures/Duration.jpeg",height=4,width=9)
```

##Proportion of time allocation
```{r}
timef %>% group_by(Month,phistate) %>% summarise(s=sum(Days)) %>% dcast(Month~phistate) %>% summarise()
```
