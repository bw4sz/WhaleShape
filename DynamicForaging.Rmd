...
ggplot(ratio,aes(y=b,x=MonthA)) + geom_boxplot() + labs(y="Proportion of Observations in Area-restricted Search",x="Month") + theme_bw()  + scale_fill_brewer(palette="RdBu") + scale_y_continuous(labels = scales::percent)
...
#remove one observation in Feb, such an outlier.

timef %>% arrange(desc(Days))

ggplot(timef,aes(x=Month,y=Days)) + geom_boxplot() + theme_bw() + facet_wrap(~phistate,scales="free") + labs(y="Bout Duration (Days)")
...
ggplot(propf[,],aes(x=Month,y=PropF)) + geom_point(size=3) + labs(y="Proportion of time in ARS") + theme_bw()
propf
...
ice$variable<-factor(ice$variable,levels=month.name[1:5])
