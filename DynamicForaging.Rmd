...
#missing argos class
mxy<-mxy[!is.na(mxy$argos.lc),]

#get rid of z
mxy<-mxy[!mxy$argos.lc=="Z",]
...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=50000,n.burnin=46000,n.thin=10,DIC=FALSE))
}
...
ggplot(timef,aes(x=Month,y=Days,fill=phistate)) + geom_boxplot() + theme_bw() + facet_wrap(~phistate,scales="free") + labs("Bout Duration (Days)")
ggsave("Figures/Duration.jpeg",height=4,width=9)
```

##Proportion of time allocation
```{r}
propf<-timef %>% group_by(Animal,Month,phistate) %>% summarise(s=sum(Days)) %>% dcast(Animal+Month~phistate,fill=0) %>% mutate(PropF=`Area-restricted Search`/(Traveling+`Area-restricted Search`),TotalTime=Traveling+`Area-restricted Search`)
ggplot(propf[propf$TotalTime > 10,],aes(x=Month,y=PropF)) + geom_boxplot() + labs(y="Proportion of individual foraging")
```
