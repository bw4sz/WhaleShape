...
timed<-function(d,step_length){
  ...
  track_time<-d %>% group_by(Track) %>% summarize(mt=difftime(max(as.POSIXct(timestamp)),min(as.POSIXct(timestamp)),units="hours")) %>% filter(mt>=16) %>% .$Track
...
ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=bstate,group=paste(Animal,Track)),size=0.8)  + borders(fill=NA,alpha=0.9) + coord_cartesian(ylim = c(-69,-60),xlim=c(-53,-72)) + theme_bw() + mytheme + scale_fill_manual(values=rep("NA",8)) + labs(col="Behavior") + scale_color_manual(values=c("grey10","grey60"))  + facet_wrap(~Month,nrow=2)
ggsave("Figures/SpatialPred_BW.jpg",height=4,width=10,nrow=1)
...
ggplot(mr,aes(x=MonthA,y=mean,ymin=lower,ymax=upper,col=MonthA)) + geom_pointrange(size=1.2,alpha=.7) + theme_bw() + ggtitle("Average Duration of Behavior") + labs(y="Hours",col="Month",x="Behavior") + facet_wrap(~bstate,scales="free")
...
ggplot(ratio,aes(x=b,fill=MonthA)) + geom_density(alpha=0.8) + labs(x="Percentage of time in Area-restricted Search state",fill="Month") + theme_bw()  + scale_fill_brewer(palette="RdBu")
```
