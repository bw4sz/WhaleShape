...
## Confidence
```{r}
ggmap(m)+geom_path(data=mxy, aes(x=x, y=y,col=ratio,group=Animal),size=.5)   + mytheme +scale_color_continuous(low='blue',high='red')
```
...
ggplot()+geom_path(data=mxy, aes(x=x, y=y,col=ratio,group=paste(Animal,Track)),size=.5) + facet_wrap(~Animal,scales="free") + mytheme +scale_color_continuous(low='blue',high='red')
```
