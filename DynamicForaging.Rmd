...
if(newModel){
  system.time(jagM<-jags.parallel(model.file = "Bayesian/Multi_RW.jags",data=data,n.chains=2,parameters.to.save=pt,n.iter=40000,n.burnin=38000,n.thin=5,DIC=FALSE))
}
...
ggplot(data=pc[pc$parameter %in% c("gamma"),],aes(y=value,x=MonthA)) + geom_boxplot() + theme_bw()  + ggtitle("Estimated parameters") + facet_wrap(~Behavior,scales="free")

ggplot(data=pc[pc$parameter %in% c("gamma"),],aes(y=value,x=MonthA)) + geom_boxplot() + theme_bw()  + ggtitle("Movement autocorrelation") + facet_wrap(~Behavior,scales="free") + labs(x="Month")

a<-pc[pc$parameter %in% c("gamma"),] %>% mutate(bstate=ifelse(Behavior==1,"Traveling ","Foraging"))
ggplot(data=a,aes(y=value,x=MonthA)) + geom_boxplot() + theme_bw()  + ggtitle("Movement autocorrelation") + facet_wrap(~bstate,scales="free") + labs(x="Month")
...
ggplot(data=a,aes(y=inv.logit(value),x=MonthA)) + geom_boxplot() + theme_bw()+ facet_wrap(~bstate,scales="free") + labs(x="Month",y="Transition Probability") + scale_fill_brewer(palette="RdBu")

ggplot(a,aes(x=MonthA,y=value)) + geom_boxplot() + facet_wrap(~bstate,scale="free") + theme_bw() + labs(x="Month",y="estimate") + ggtitle("Transitions among behaviors")
...
ggplot(prop,aes(x=as.factor(value),y=s,fill=MonthA)) + geom_violin() + labs(x="Behavior",y="Hours",fill="Month") + scale_x_discrete(label=c("Traveling","Area-restricted Search"))
