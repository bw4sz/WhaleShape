...
distbout<-function(a){
  ...
  Index<-c(1,cumsum(stater$lengths))
  
  #if(length(Index)==2){return(data.frame(NULL))}
  
  #label bouts
  a$Bout<-NA
  for(x in 1:(length(Index)-1)){
    a$Bout[Index[x]:Index[x+1]]<-x
  }
  
  #we don't know what happens before or after a track.
  a<-a %>% filter(!Bout %in% c(min(Bout),max(Bout)))
...
#Time between bouts: Method II

```{r,eval=F}
#function for calculating points in a track
tracktime<-function(a){
  return(data.frame(Days=difftime(max(a$timestamp),min(a$timestamp))))
}
...
timebout<-function(a){
  ...
  #for each traveling bout calculate the trackdist
  sumbout<-a %>% group_by(Bout) %>% do(tracktime(.))
  
  #label each bout by the month at the beginning of the bout
  return(a %>% group_by(Bout) %>% summarize(MonthF=min(MonthF),phistate=unique(phistate)) %>% inner_join(sumbout))
  }
  
#calculate distance between foraging bouts for all tracks
timef<-mxy %>% group_by(Animal,Track) %>% do(timebout(.))

#label months as factors
timef$Month<-month.name[timef$MonthF]
timef$Month<-factor(distf$Month,levels=month.name)

ggplot(timef,aes(x=Month,y=Days)) + geom_boxplot() + theme_bw()
```
